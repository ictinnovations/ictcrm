                ...
                foreach ($panel as $rowCount=>$row) {
                    foreach ($row as $key=>$column) {
                        if ($this->matches($column, '/^date_entered$/') || $this->matches($column, '/^created_by$/')) {
                            ...
                            if ($this->matches($column, '/^team.*?(_name)?$/s')) {
                                $panels[$name][$rowCount][$key] = 'team_name';
                            } else {
                                if ($this->matches($column, '/^date_modified$/') || $this->matches($column, '/^modified_by$/')) {
                                    $panels[$name][$rowCount][$key] = array(
                                                            'name' => 'date_modified',
                                    ...
                                    if ($this->matches($column, '/^assigned.*?(_to|_name|_link)$/s')) {
                                        ...
                                        if ($this->matches($column, '/^vcard_link$/')) {
                                            ...
                                            if ($this->matches($column, '/^parent_type$/si')) {
                                                $panels[$name][$rowCount][$key] = 'parent_name';
                                            } else {
                                                if ($this->matches($column, '/^account_id$/')) {
                                                    $panels[$name][$rowCount][$key] = 'account_name';
                                                } else {
                                                    if ($this->matches($column, '/^contact_id$/')) {
                                                        $panels[$name][$rowCount][$key] = 'contact_name';
                                                    } else {
                                                        if ($this->matches($column, '/^reports_to_id$/')) {
                                                            $panels[$name][$rowCount][$key] = 'report_to_name';
                                                        } else {
                                                            if ($this->matches($column, '/^reminder_time$/')) {
                                                                $panels[$name][$rowCount][$key] = array(
                                                           'name'=>'reminder_checked',
                                                           'fields'=>array('reminder_checked', 'reminder_time')
                                                           );
                                                            } else {
                                                                if ($this->matches($column, '/^currency(_name)*$/')) {
                                                                    $panels[$name][$rowCount][$key] = 'currency_id';
                                                                } else {
                                                                    if ($this->matches($column, '/^quote_id$/')) {
