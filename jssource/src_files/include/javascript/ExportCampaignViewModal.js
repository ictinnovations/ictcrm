...
(function($) {
$.fn.ExportCampaignViewModal=function(options) {
"use strict";
var self=this;
var opts=$.extend( {

},$.fn.ExportCampaignViewModal.defaults,options);
self.handleClick=function(e) {
"use strict";
var self=this;
self.exportCampaignView=null;
var opts=$.extend( {

},$.fn.ExportCampaignViewModal.defaults);
var composeBox=$('<div></div>').appendTo(opts.contentSelector);
composeBox.messageBox( {
"showHeader":false,"showFooter":false,"size":'lg'
});
composeBox.setBody('<div class="email-in-progress"><img src="themes/'+SUGAR.themes.theme_name+'/images/loading.gif"></div>');
composeBox.show();
$.ajax( {
type:"GET",cache:false,url:'index.php?module=Leads&action=CampaignView&in_popup=1'
}).done(function(data) {
if(data.length===0) {
...
self.exportCampaignView=composeBox.controls.modal.body.find('.compose-view').ExportCampaignView();
$(self.exportCampaignView).on('sentEmail',function(event,composeView) {
composeBox.hide();
composeBox.remove();

});
$(self.exportCampaignView).on('disregardDraft',function(event,composeView) {
...
$.fn.openCampaignViewModal=function(source) {
"use strict";
...
var url='index.php?module=Leads&action=CampaignView&in_popup=1&targetModule='+targetModule+ids+'&relatedModule='+currentModule+'&relatedId='+relatedId;
$.ajax( {
...
if(targetCount>0) {
$(self.exportCampaignView).find('#to_addrs_names').val(targetList);

}
if(targetCount<2) {
$(self.exportCampaignView).find('#parent_type').val(populateModule);
$(self.exportCampaignView).find('#parent_name').val(populateModuleName);
$(self.exportCampaignView).find('#parent_id').val(populateModuleRecord);
...
mb.setTitle(SUGAR.language.translate('','LBL_CONFIRM_DISREGARD_EXPORT_TITLE'));
mb.setBody(SUGAR.language.translate('','LBL_CONFIRM_DISREGARD_EXPORT_BODY'));
mb.on('ok',function() {
mb.remove();
composeBox.hide();
composeBox.remove();
...
composeBox.controls.modal.content.html(SUGAR.language.translate('','LBL_EXPORT_ERROR_GENERAL_TITLE'));
...
$.fn.ExportCampaignViewModal.defaults= {
'selected':'INBOX','buttonSelector':'[data-action=emails-show-compose-modal]','contentSelector':'#content'
};

}(jQuery));
/* End of File jssource/src_files/include/javascript/ExportCampaignViewModal.js */
