  ...
  if (xmlhttp.readyState == 4) {
    if (xmlhttp.status == 200 || xmlhttp.status == 0) {
      ...
      for (var i = 0; i < acc.length; i++) {
        ...
        SUGAR.util.globalEval("users_array[users_array.length] = (" + val + ")");
      }
    }
  }
}
...
//	new_input.accessKey="G";
  ...
  new_input.onclick = function () {
    ...
    return open_popup(module_name, 600, 400, "", true, false, {
      "call_back_function": "set_form_return_reports",
      "form_name": "EditView",
...
function joinChecked(obj) {
  ...
  for (var index in filters_arr) {
    ...
    if (objName.indexOf("_div") > -1 && objName.substr(0, objName.indexOf("_div")) == filter_module) {
      ...
      // Bug 13124////
      var filter = {
        column_name: filter_row.module_select[filter_row.module_select.selectedIndex].value,
...
function getAllFieldsMapped(module) {
  ...
  for (var k in module_defs[module].field_defs) {
    all_fields["self:" + module_defs[module].field_defs[k].name] = {
      "field_def": module_defs[module].field_defs[k],
  ...
  for (var k in module_defs[module].summary_field_defs) {
    all_fields["self:" + module_defs[module].summary_field_defs[k].name] = {
      "field_def": module_defs[module].summary_field_defs[k],
      "linked_field_name": "self",
      "label_prefix": module_defs[module].label
    };
  ...
  for (var i in link_defs) {
    ...
    for (var j in module_defs[join_module].field_defs) {
      all_fields[i + ":" + module_defs[join_module].field_defs[j].name] = {
        "field_def": module_defs[join_module].field_defs[j],
        "linked_field_name": i,
    ...
    for (var j in module_defs[join_module].summary_field_defs) {
      ...
      all_fields[i + ":" + module_defs[join_module].summary_field_defs[j].name] = {
        "field_def": module_defs[join_module].summary_field_defs[j],
        "linked_field_name": i,
        "label_prefix": link_defs[i].label
      };
    }
