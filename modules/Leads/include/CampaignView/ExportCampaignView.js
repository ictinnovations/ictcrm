...
(function ($) {
   ...
   * @constructor
   */
  $.fn.ExportCampaignView = function (options) {
console.log("call.");
    "use strict";
    var self = $(this);
    var opts = $.extend({}, $.fn.ExportCampaignView.defaults, options);
     ...
     * $('.compose-view#id).ExportCampaignView().qtipBar +=
     ...
     * $('.compose-view#id).ExportCampaignView().qtipBar += '<button class="btn btn-default btn-sm"></button>';
     *
     * @type {string}
     */
    self.qtipBar =
    ...
    self.handleQTipBarClick = function () {
      ...
      $.fn.ExportCampaignView.setEmailAddressFieldFromPopup = function(resultData) {
      ...
      var popupWindow = open_popup(
        ...
        false,
        {
          "call_back_function": '$.fn.ExportCampaignView.setEmailAddressFieldFromPopup',
    ...
    $.fn.ExportCampaignView.updateSignature = self.updateSignature = function () {
    ...
    self.sendEmail = function (e) {
      ...
      return sListView.send_form(true, currentModule, 'index.php?entryPoint=export','Please select at least 1 record to proceed');
//retrun true;
    };
    ...
    self.attachDocument = function (event) {
      ...
      $.fn.ExportCampaignView.selectDocumentFromPopup = function(resultData) {
      ...
      var showSelectDocumentDialog = function () {
        ...
        var popupWindow = open_popup(
          ...
          false,
          {
            "call_back_function": '$.fn.ExportCampaignView.selectDocumentFromPopup',
            "form_name": "ComposeView",
            "field_to_name_array": {
    ...
    self.onSavingDraft = function () {
      ...
      $.ajax({
        ...
        if (typeof response.errors !== "undefined") {
          ...
          $.fn.ExportCampaignView.checkForDraftAttachments(response.data.id);
        }
      }).fail(function (response) {
        "use strict";
        response = JSON.parse(response);
    ...
    self.construct = function () {
      ...
      if (self.length === 0) {
        console.error('ExportCampaignView - Invalid Selector');
        return;
      }

      if (self.attr('id').length === 0) {
        console.warn('ExportCampaignView - expects element to have an id. ExportCampaignView has generated one.');
        self.attr('id', self.generateID());
      }

      if (self.find("[name=record]").val().length > 0) {
        $.fn.ExportCampaignView.checkForDraftAttachments(self.find("[name=record]").val());
      }
      ...
      if ($(self).find('#from_addr_name').length !== 0) {
        ...
        $.ajax({
          ...
          if (typeof json.errors !== "undefined") {
            $.fn.ExportCampaignView.showAjaxErrorMessage(json);
          }
        }).error(function (response) {
      ...
      if (typeof tinymce === "undefined") {
        console.error('ExportCampaignView - Missing Dependency: Cannot find tinyMCE.');
        ...
        var intervalCheckTinymce = window.setInterval(function () {
          var isFromPopulated = $('#ict_campaign_id').prop("tagName").toLowerCase() === 'select';
          var ictbroadcast_campaign_id = $(self).find('select[name="ict_campaign_id"]').val();
          document.cookie = "ictCampaignid="+ictbroadcast_campaign_id;
      ...
      $(self).trigger("constructExportCampaignView", [self]);
    };
  ...
  $.fn.ExportCampaignView.checkForDraftAttachments = function (id) {
    ...
    $.ajax({
      ...
      if (typeof response.data !== "undefined") {
        ...
        $.fn.ExportCampaignView.loadAttachmentDataFromAjaxResponse(response);
      }
      if (typeof response.errors !== "undefined") {
        $.fn.ExportCampaignView.showAjaxErrorMessage(response);
      }
    }).error(function (response) {
      console.error(response);
    });
  ...
  $.fn.ExportCampaignView.showAjaxErrorMessage = function (response) {
    var message = '';
  ...
  $.fn.ExportCampaignView.loadAttachmentDataFromAjaxResponse = function (response) {
  ...
  $.fn.ExportCampaignView.onTemplateSelect = function (args) {
    ...
    var confirmed = function (args) {
      ...
      $.post('index.php?entryPoint=emailTemplateData', {
        ...
        $.fn.ExportCampaignView.updateSignature();
      });
      set_return(args);
    };
  ...
  $.fn.ExportCampaignView.onParentSelect = function (args) {
    set_return(args);
  ...
  $.fn.ExportCampaignView.defaults = {
    "tinyMceOptions": {
      menubar: false,
