...
class LeadsViewCompose extends ViewEdit
{
    ...
    public function preDisplay()
    {
        ...
        $this->ev->setup(
            ...
            $this->bean,
            $metadataFile,
            get_custom_file_if_exists('modules/Leads/include/CampaignView/CampaignView.tpl')
        );
    }
    ...
    {
        $a = dirname(dirname(__FILE__)) . '/include/CampaignView/CampaignView.php';
        require_once 'modules/Leads/include/CampaignView/CampaignView.php';
    ...
    public function getSignatures(User $user)
    {
        if (empty($user->id) || $user->new_with_id === true) {
            throw new \SugarControllerException(
                'LeadsController::composeSignature() requires an existing User and not a new User object. '.
        ...
        $GLOBALS['log']->warn(
            'LeadsController::composeSignature() was unable to get the signature id for user: '.
                $user->name
            );
        return false;
    }
