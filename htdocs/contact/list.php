...
//print_fiche_titre($langs->trans("Ictcore"),false,'setup');
//include_once("./core/class/html.formictcore.class.php");
require_once DOL_DOCUMENT_ROOT.'/custom/ictcore/core/class/html.formictcore.class.php';

$formsms = new formictcore($db);
...
$formsms->param["smsid"]=0;
 $formsms->param["returnurl"]=$_SERVER['REQUEST_URI'];//DOL_URL_ROOT.'/custom/ictcore/send.php?contact=list';
$action=GETPOST('action');
 if ($action == 'send' && ! $_POST['cancel'])
{

	//exit;
    require_once DOL_DOCUMENT_ROOT.'/custom/ictcore/campaignaction.php';
...
if (! GETPOST('confirmmassaction', 'alpha') && $massaction != 'presend' && $massaction != 'confirm_presend' && $massaction != 'ictcore') { $massaction=''; }
...
if($massaction != 'ictcore'){
include DOL_DOCUMENT_ROOT.'/core/tpl/massactions_pre.tpl.php';
}else{
	//echo $massaction;exit;
	print '</form>';
	//echo "adeel bhutta";
	dol_fiche_head(null, '', '');
	//print_r($arrayofselected);
	$formsms->selected_contact_ids = $arrayofselected;
	$formsms->show_form();
	//echo "adeel bhutta";
	dol_fiche_end();
}
