...
function dol_print_phone($phone, $countrycode = '', $cid = 0, $socid = 0, $addlink = '', $separ = "&nbsp;", $withpicto = '', $titlealt = '', $adddivfloat = 0)
{
	...
	//echo $phone.$countrycode.$withpicto.$titlealt;
	...
	if (! empty($addlink))	// Link on phone number (+ link to add action if conf->global->AGENDA_ADDACTIONFORPHONE set)
	{
		...
		elseif (! empty($conf->clicktodial->enabled) && $addlink == 'AC_TEL')		// If click to dial, we use click to dial url
		{
			...
			if(isset($_GET['id']) && $_GET['id']!='' ){
				$urlmask='?id='.$_GET['id'].'&ictcore=&click=yes&phone='.$newphone.'&mode=init#ICTCore';
			}else{
				$urlmask='?ictcore=&click=yes&phone='.$newphone;
			}
			//if(empty($query_string) || $query_string==''){
				
			//}
			  if(isset($_GET['click']) && $newphone==$_GET['phone']){
					//echo $query_string;
					//echo $urlmask;
...
<!-- 					   <button type="button" class="btn btn-info btn-lg" data-toggle="modal" data-target="#myModal">Open Modal</button>
 -->
					 <!-- Modal -->
					  <div class="modal fade" id="<?php echo $newphone; ?>" role="dialog">
					    <div class="modal-dialog">
					      ...
					      <div class="modal-content">
					        <div class="modal-header">
					          ...
					          <h4 class="modal-title">Select Voice Message</h4>
					        </div>
					        ...
					        <?php 
					          	...
					          	$url      = $conf->global->ICTCore_ICTURL;    
				                $username = $conf->global->ICTCore_ICTUSERNAME;    
				                $password = $conf->global->ICTCore_ICTPASSWORD;    
				                $apiictcore      = new IctCoreApi($url,$username,$password);

				                 $getmvoices = $apiictcore->getrecording();
					          		
					          		//print '<tr id="old_m" style="display:none"  ><td width="180">';
						            // print $langs->trans("Voice Message").'</td><td>';
						            
						            print '<form method="post" action="'.$_SERVER["REQUEST_URI"].'" name="formfilter" enctype="multipart/form-data">';

						            //print "<form id='icttransmission' method='post' action='".DOL_URL_ROOT."/custom/ictcore/send.php'>";
						            print "<input type='hidden' id='phone_nm' name='phone_no' value='".$newphone."'>";
						            print "<input type='hidden' name='ictocre' id='actio' value='tranmission'>";
						            print "<select name='ict_voice_message' id='ict_voice_message' >
						                     <option value=''>please select option</option>";
						                    foreach($getmvoices as $rcording){
						                      print "<option value='".$rcording->recording_id."'>".$rcording->name."</option>";
						                    }
						                    
						                     print "</select>";
						            print "</form>"; 
						            $message='<div class="ok">'."Transmission successfully created".'</div>';
						            print "<div id='show_s_message' style='display:none'>".dol_htmloutput_mesg($message)."</div>";    
						           	$messagee='<div class="error">'."some thing wrong".'</div>';


						            print "<div id='show_e_message' style='display:none'>".dol_htmloutput_mesg($messagee)."</div>";    

						             //print '</td></tr>';

					          ?>
					          <div id="page-loadw"  style="display:none">please waite your transmission created<img src="http://loadinggif.com/images/image-selection/29.gif" /></div> 
					  ...
					  <script type="text/javascript">
					  	$(document).ready(function() {
					  		 $('#<?php echo $newphone; ?>').modal('show')
					  		 	
					  		 	
					  		});
					  			$('#ict_voice_message').change(function (event) {
								 //$('#myModal').modal('show')
								 //alert('test');
                                  //alert();
									...
									if(getUrl.host=='localhost'){
								 	...
								 	var phone      = $("#phone_nm").val();
								 	var actio      = $("#actio").val();
									var l_id 	   = "<?php echo $_GET['id'] ?>";
									var formData = {id:message_id,phone:phone,actio:actio,l_id:l_id };
									  	
									  	if (confirm("Are you sure you wish to create tranmission?")) {
										     //alert(message_id);
										     $('#page-loadw').show();
										     //set back
										    $.ajax({ //Process the form using $.ajax()
													type      : 'POST', //Method type
													url       : baseUrl+'/htdocs/custom/ictcore/ajaxupload.php', //Your form processing file URL
													...
													success   : function(data) {
														...
														alert(getUrl.pathname.split('/')[3]);
														alert(getUrl.pathname.split('/')[4]);*/
														if(data==200){
															$("#show_s_message").show();
															$("#show_e_message").hide();
															alert("Transmission created Successfully");
															// Sets the new location of the current window.
															//window.location = getUrl .protocol + "//" + getUrl.host + "/" + getUrl.pathname.split('/')[1]+ "/" + getUrl.pathname.split('/')[2]+ "/" + getUrl.pathname.split('/')[3]+ "/" + getUrl.pathname.split('/')[4];
															//window.location = "<?php echo $_SERVER['REQUEST_URI']; ?>";
															window.location = "<?php echo strtok($_SERVER['REQUEST_URI'],'&'); ?>";
															...
															$("#show_s_message").hide();
															$("#show_e_message").show();
															alert("Whoops! Transmission not created some thing wrong");
														}
														 $('#page-loadw').hide();
													},
								 	...
								 	//$( "#icttransmission" ).submit();
								 	//document.getElementById("icttransmission").submit()
