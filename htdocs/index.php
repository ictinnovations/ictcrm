...
if (empty($user->societe_id))
{
    ...
    if (empty($reshook))
    {
		...
		! empty($conf->ictcore->enabled) && $user->rights->ictcore->campaigns,
		! empty($conf->ictcore->enabled) && $user->rights->ictcore->campaigns,
	    );
	    ...
	    $includes=array(
			...
			DOL_DOCUMENT_ROOT."/custom/ictcore/class/ict_campaign.class.php",
			DOL_DOCUMENT_ROOT."/custom/ictcore/class/ict_transmission.class.php"
	    );
	    ...
	    $links=array(
			...
			DOL_URL_ROOT.'/custom/ictcore/list.php?leftmenu=ictcampaigns',
			DOL_URL_ROOT.'/custom/ictcore/list.php?leftmenu=icttransmissions'
	    );
	    ...
	    foreach ($keys as $key=>$val)
	    {
	        ...
	        if ($conditions[$key])
	        {
	            ...
	            if (! isset($boardloaded[$classe]) || ! is_object($boardloaded[$classe]))
	            {
	                ...
	                //echo "<br>".$val."<br>";
	                //echo "<pre>";print_r( $board->load_state_board($user));
	                $boardloaded[$classe]=$board;
	            }
	            else
	            {
	                $board=$boardloaded[$classe];
	                 //echo "<pre>";print_r($boardloaded[$classe]);
	            }


	            if (!empty($langfile[$key])) 
